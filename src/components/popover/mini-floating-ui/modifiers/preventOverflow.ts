...
function preventOverflow({ state, options, name }: ModifierArguments<Options>) {
  ...
  if (checkMainAxis || checkAltAxis) {
    ...
    const minOffset = isBasePlacement ? referenceRect[len] / 2 - additive : minLen
    const maxOffset = isBasePlacement ? -referenceRect[len] / 2 + additive : maxLen
    ...
    if (checkAltAxis) {
      ...
      const preventedOffset = within(Math.min(min, tetherMin), offset, Math.max(max, tetherMax))
