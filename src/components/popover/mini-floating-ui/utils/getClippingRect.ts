...
function getClippingParents(element: Element): Array<Element> {
  ...
  const clipperElement =
    canEscapeClipping && isHTMLElement(element) ? getOffsetParent(element as HTMLElement) : element
...
// clipping parents
// 把所有元素，默认是 popper 的父 scroll 元素，都计算一下他们所有的 top，right，bottom，left 的最小区域
// 在 CSS 坐标系中：
// - top : 距离顶部的距离（越小越靠上）
// - left : 距离左边的距离（越小越靠左）
// - right : 距离左边的距离（越大越靠右）
// - bottom : 距离顶部的距离（越大越靠下）
